...
exports.main = function() {
  ...
  tabs.on("ready", function(tab){
    ...
    worker.port.on("handshake", function(detail){
      ...
      var gm_pref = "extensions.greasemonkey.scriptvals." + namespace + "/" + name + ".handshake";
      ...
      if(gm_data) {
        ...
        worker.port.on("fireipc_send", function(message){
          console.log("US_msg", message.name, message.uuid, message.detail.msg.data);
          var data = JSON.stringify(message.detail.msg) + "\n";
  ...
  try  {
    ...
    serverSocket.init(61155, true, -1);
          ...
          pump.asyncRead(
            ...
            onDataAvailable: function (request, context, stream, offset, count){
                ...
                for(var i in workers){
                  ...
                  try {
                    worker.port.emit("fireipc_recive", data);
                  } catch(e){
                    console.log("Error" + e.result + ": " + e.message);
                  }
                }
